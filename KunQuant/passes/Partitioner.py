...
from KunQuant.ops.MiscOp import Accumulator, SetAccumulator, WindowedLinearRegressionConsumerTrait, WindowedLinearRegression
...
def _is_non_output_op(op: OpBase) -> bool:
    ...
    '''
    if op.get_parent() is not None or isinstance(op, WindowedLinearRegression) or isinstance(op, BoolOpTrait) or isinstance(op, Accumulator):
        return True
    return False
...
def _is_fast_select_op(op: OpBase) -> bool:
    ...
    '''
    if isinstance(op, ReductionOp) or isinstance(op, WindowedLinearRegressionConsumerTrait) or isinstance(op, SetAccumulator) or _is_non_output_op(op):
        return True
    else:
