...
def _is_non_output_op(op: OpBase) -> bool:
    ...
    '''
    if op.get_parent() is not None or isinstance(op, (GlobalStatefulProducerTrait, BoolOpTrait)):
        return True
    return False
...
def _is_fast_select_op(op: OpBase) -> bool:
    ...
    '''
    if isinstance(op, (ReductionOp, StateConsumerTrait, ReturnFirstValue)) or _is_non_output_op(op):
        return True
    else:
